...
class Codido:
	...
	def svstotiff(self,file_name):
		...
		print('img read done')	
		basename=Path(file_name).stem
		out_image_path=self.input_folder_path + basename+'.tif'
		#os.remove(file_name)
		#img.save(out_image_path) #create png file
		tifffile.imwrite(out_image_path, img, photometric='rgb',bigtiff=True)
		#os.remove(file_name) #remove svs file
		file_name = out_image_path
		return file_name
